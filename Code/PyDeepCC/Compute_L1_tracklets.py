...
def compute_L1_tracklets(total_frame):
    ...
    for window_start_frame in range(0, total_frame, params['window_width']):
        ...
        window_inds = np.where(list(map(lambda x:x in window_frames, all_dets[:, frame_index])))
        ...
        vaild, detections_in_window = get_valid_detections(detections_in_window, detections_conf, num_visiable)
        detections_in_window = detections_in_window[vaild, :]
        filtered_detections = None


        filtered_features = features[window_inds[vaild]]
        tracklets = create_tracklets(params, filtered_detections, filtered_features,
